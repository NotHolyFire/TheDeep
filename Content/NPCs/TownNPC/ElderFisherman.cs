...
namespace TheDeep.Content.NPCs.Friendly.TownNPC
{
    ...
    public class ElderFisherman : ModNPC
    {
        ...
        {
            new NPCShop(Type)
                .Add(new Item(ModContent.ItemType<Electronic>()) { shopCustomPrice = Item.buyPrice(copper: 15) })
                .Add(new Item(ModContent.ItemType<BrassFishingRod>()) { shopCustomPrice = Item.buyPrice(platinum: 9999) })
                .Register();
        }
        ...
        {
            projType = ModContent.ProjectileType<TideInABall>();
            attackDelay = 1;
        }
    }
}