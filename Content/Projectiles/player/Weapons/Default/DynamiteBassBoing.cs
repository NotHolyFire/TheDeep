...
namespace TheDeep.Content.Projectiles.player.Weapons.Default
{
    // This is a basic item template.
    // Please see tModLoader's ExampleMod for every other example:
    // https://github.com/tModLoader/tModLoader/tree/stable/ExampleMod
    public class DynamiteBassBoing : ModProjectile
    {
        // This projectile demonstrates exploding tiles (like a bomb or dynamite), spawning child projectiles, and explosive visual effects.
            ...
            public override bool OnTileCollide(Vector2 oldVelocity)
            {
                ...
                if (Projectile.soundDelay == 0)
                {
                    ...
                    SoundStyle impactSound = new SoundStyle($"{nameof(TheDeep)}/Content/Assets/Sounds/boing")
                    {
                        Volume = 0.7f,
                        PitchVariance = 0.5f,
                    };
                ...
                if (Projectile.velocity.X != oldVelocity.X && Math.Abs(oldVelocity.X) > 0.6f)
                {
                    Projectile.velocity.X = oldVelocity.X * -0.5f;
                }
                if (Projectile.velocity.Y != oldVelocity.Y && Math.Abs(oldVelocity.Y) > 0.6f)
                {
